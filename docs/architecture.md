...
React Cordis 的产品本体是 `packages/` 中的 React 应用基础设施：插件构建与启动、依赖和生命周期、UI 插槽、路由、国际化与主题契约。应用宿主拥有业务数据、页面布局、导航、设计系统和平台能力；这些内容不进入通用包。

`examples/` 是基础设施的验证资产。Basic 覆盖最小启动与插槽，Router 覆盖业务布局、路由与设置扩展，i18n 覆盖语言切换与资源生命周期；多端、TanStack Start 和 Next.js 分别验证不同宿主与 SSR 边界。示例中的业务页面和收藏功能不是基础包能力，也不是项目的产品方向。
...
Router 示例的 `LayoutController` 提交快照后，按本轮订阅列表通知所有观察者；单个订阅者的同步异常会记录到 `console.error`，其余订阅者继续执行。取消订阅影响后续轮次，回调内再次修改布局会同步启动新一轮通知，观察者读取最新快照。未变化的布局保持快照引用稳定且不发送通知。
