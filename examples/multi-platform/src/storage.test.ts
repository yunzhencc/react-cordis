...
import * as browserStorage from '@examples/multi-platform-browser-storage';
import * as desktopStorage from '@examples/multi-platform-desktop-storage';
import * as fileStorage from '@examples/multi-platform-file-storage';
import * as nativeStorage from '@examples/multi-platform-native-storage';
...
it.each(['browser', 'native', 'file', 'desktop'] as const)('%s preserves named documents across handle and provider restarts', async (platform) => {
  const ctx = new Context();
  const directory = await mkdtemp(join(tmpdir(), 'cordis-storage-contract-'));
  ctx.provide('storageDirectory', directory);
  ...
  if (platform === 'desktop') {
    vi.stubGlobal('productStorage', {
      read: async (name: string) => localStorage.getItem(`cordis-cross-platform:${name}`),
      write: async (name: string, value: string) => localStorage.setItem(`cordis-cross-platform:${name}`, value),
    });
  }
