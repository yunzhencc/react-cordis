...
import type { Storage, StorageBackend, StorageDocument } from '@examples/multi-platform-storage';
import { mkdir, readFile, rename, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { provideStorage, validateStorageName, validateStorageValue } from '@examples/multi-platform-storage';

export const name = 'file-storage';
export const inject = ['storageDirectory'];
...
export function apply(ctx: Context) {
  const directory = ctx.storageDirectory;
  provideStorage(ctx, {
    async read(name) {
      try {
