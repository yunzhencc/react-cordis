...
注册过程中抛错会回滚本次已添加的字典，并保留原始错误；已有或重入注册的字典不受影响。释放时即使资源事件监听器抛错，也会继续清理该次注册的其余字典，最后抛出第一个错误。
...
并发切换按调用顺序保存并生效，前一次失败不阻塞后续请求。语言定义在排队或保存期间被卸载、替换时，该次切换会失败，不会激活旧定义；已经完成的宿主保存无法撤回。运行时释放后，已开始的保存可以完成，但不再更新语言，也不再启动排队中的保存。没有宿主保存回调时，语言仍在调用时立即生效。
